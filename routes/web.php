...
Route::group([
    ...
    Route::post('storetimelimit', 'BpackageController@storetimelimit')->name('storetimelimit');
    Route::get('openstore', 'MemberRequestController@index');
    Route::get('allstores', 'MemberRequestController@allstores');
    Route::patch('acceptstore/{id}', 'MemberRequestController@acceptstore')->name('acceptstore');
    Route::patch('declinestore/{id}', 'MemberRequestController@declinestore')->name('declinestore');
...
Route::group([
    ...
    Route::group(['prefix' => 'ajax'], function($router) {
        ...
        Route::post('category/{id}', 'Ajax\CategoryController@allSubCat');
		Route::post('categoryy/custom-fields', 'Ajax\CategoryController@getCustomFields');
...
Route::group([
    ...
    Route::group(['middleware' => ['web']], function($router) {
        ...
        Route::group(['namespace' => 'Post'], function($router) {
            ...
            Route::get('{title}/{id}/promote', 'PromoteController@index');
            Route::get('{title}/{id}/promote/bkashpayment', 'PromoteController@payment')->name('bkashpayment');
            Route::post('adpromote', 'PromoteController@adpromote')->name('promotion');
            Route::post('storebkashpayment', 'PromoteController@bkashpayment')->name('storebkashpayment');
            Route::get('paymentcomplete', 'PromoteController@paymentcomplete')->name('paymentcomplete');
            // Send report abuse
        ...
        Route::group(['middleware' => ['auth', 'bannedUser', 'preventBackHistory'], 'namespace' => 'Account'], function($router) {
            ...
            Route::post('storeinfo', 'StoreController@store');
            Route::patch('storeinfo/{id}', 'StoreController@update')->name('shopinfoupdate');
            Route::get('get-membership','StoreController@getMembership');
            Route::patch('bemember/{id}','StoreController@beMember')->name('bemember');
