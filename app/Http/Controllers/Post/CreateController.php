...
class CreateController extends FrontController
{
    ...
    public function postForm($tmpToken = null, PostRequest $request)
    {
        ...
        $postInfo = [
            ...
            'uniq_id'        => $this->generateUniqNumber(),
    ...
    {
        $number =substr(md5(uniqid(rand(1,6))), 0, 8);
        if ($this->uniqIdExists($number)) {
            return generateUniqNumber();
        }
        return $number;
    }
    public function uniqIdExists($number) {
        return Post::whereuniq_id($number)->exists();
    }
