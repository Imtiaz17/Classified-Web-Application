...
class PackageController extends FrontController
{
    ...
    public function commonQueries()
    {
        ...
        // Get Packages
        $this->packages = BPackage::orderBy('id')->get();
     ...
     */
    public function postForm($postIdOrToken, Request $request)
    {
        // Get Post
        if (getSegment(2) == 'create') {
            if (!session()->has('tmpPostId')) {
        ...
        foreach ($package as $key => $value) {
                $durations[]=Duration::where('package_id','=',$value)
                            ->where('duration','=',$duration[$key])->first();
            }
        $totalprice= $request->totalprice;
        $promotion=[];
		foreach($durations as $key => $result){
			$promotion[] = new Promotion([
				'post_id'=>$request->postid,
				'package_name'=>$result->package_id,
				'duration'=>$result->duration,
				'duration_price'=>$result->price,
				'total_price'=>$totalprice,
				'bkash_number'=>$request->bkash_no,
				'transaction_id'=>$request->ref_no,
				
			]);
		
         }
	     $post->promotions()->saveMany($promotion);
