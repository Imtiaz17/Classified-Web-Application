...
@if (isset($posts) and $posts->getCollection()->count() > 0)
	...
	@if($post->featured==0 or $post->bumpad==1)
		{{-- expr --}}
	<div class="make-list" style="width: 100%;clear: both;overflow: hidden;border-bottom: 1px solid #dcddda;padding: 8px 0px;">
		...
		@if ($post->bumpad==1)
			{{-- expr --}}
		<div class="ads-icon" >
			<a href="#"><img width="20" src="{{asset('images/dailyad.png')}}"></a>
		</div>
		@endif
	</div>
	@endif
		...
		<?php if (($key + 1) % 4 ==0): ?>
			<p style="padding: 6px 44px;
    margin: 0px;
    background: #ccc;">Google Ads</p>
		<?php
